...
import yaml
import secrets
from flask import render_template, make_response, send_from_directory, send_file, flash, redirect, request, abort
...
def contest_problems(problems):
    ...
    solved = []
    try:
        solved = settings.find_one(
            {"type": "profile", "name": current_user.name})['solved']
    except:
        pass

    for problem in settings.find({"type": "problem", "contest": contest}):
        if not perms(problem, current_user.name):
            problems.append((problem['name'], problem['name'] in solved, problem['points'], ", ".join(
                problem['types']), ", ".join(problem['authors'])))
...
def problems():
    ...
    if not contest:
        problems = sorted([(x['name'], x['points'], ", ".join(x['types']), ", ".join(x['authors']))
                          for x in settings.find({"type": "problem", "published": True})], key=cmp_to_key(extras.cmpProblem))
...
def private_problems():
    ...
    for x in settings.find({"type": "problem", "published": False}):
        if not extras.perms(x, current_user.name):
            arr.append((x['name'], x['points'], x['contest'],
                       ", ".join(x['types']), ", ".join(x['authors'])))
...
def viewProblem(problemName):
    ...
    src = None
    try:
        bucket.blob("ProblemStatements/" + problemName +
                    ".txt").download_to_filename("statement.md")
        src = open("statement.md", "r").read()
    except:
        src = "This problem does not yet have a problem statement."

    bucket.blob("TestData/" + problemName +
                "/resources.yaml").download_to_filename("resources.yaml")
...
def submit(problemName):
    ...
    if form.validate_on_submit():
        ...
        lang = form.lang.data
        src = form.src.data
        settings.insert_one({"type": "submission", "problem": problemName,
                            "author": current_user.name, "lang": lang, "message": src, "id": sub_cnt, "output": ""})
        ...
        rpc = Process(target=runSubmission, args=(
            judges, current_user.name, src, lang, problemName, False, return_dict, sub_cnt,))
        rpc.start()
...
def raw_submission(sub_id):
    ...
    if not sub:
        ...
        abort(403)
    output = sub['output'].replace("diff", "").replace("`", "").replace(
        "+ ", "  ").replace("- ", "  ").replace(" ", "%sp%").strip().replace("\n", "%nl%")
...
def raw_source(sub_id):
    ...
    # , title="Raw source from " + str(sub_id), src = sub['message'].strip())
...
def upload_file():
    ...
    if uploaded_file.filename != '':
        ...
        try:
            msg = problem_uploading.uploadProblem(
                settings, storage.Client(), current_user.name)
            flash(msg, "success")
